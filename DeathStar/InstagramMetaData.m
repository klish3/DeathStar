...
@implementation InstagramMetaData
...
- (void)setPhoto:(NSDictionary *)photo {
    ...
    NSNumber *numberOfComments = [_photo valueForKeyPath:@"comments.count"];
    [self.commentsButton setTitle:[NSString stringWithFormat:@"%@",numberOfComments] forState:UIControlStateNormal];
...
- (UIImageView *)avatarImageView {
    if (!_avatarImageView) {
        ...
        _avatarImageView.layer.borderColor = [[self class] gourmetTextColor].CGColor;
...
- (UIButton *)timeButton {
    if (!_timeButton) {
        _timeButton = [[UIButton alloc] initWithFrame:CGRectMake(490.0f, 0.0f, 50.0f, 32.0f)];
...
- (UIButton *)likesButton {
    if (!_likesButton) {
        _likesButton = [[UIButton alloc] initWithFrame:CGRectMake(10.0f, 600.0f, 50.0f, 40.0f)];
...
- (UIButton *)commentsButton {
    if (!_commentsButton) {
        _commentsButton = [[UIButton alloc] initWithFrame:CGRectMake(490.0f, 600.0f, 50.0f, 40.0f)];
...
+ (UIColor *)gourmetTextColor {
    return [UIColor colorWithRed:0.128f green:0.209f blue:0.116f alpha:1.0f];
}
